...
- `src/`: library source — pattern query core (`pattern.ts`) with exact rational time (`fraction.ts`), mini-notation parser (`mini.ts`), note()/sound()/n() constructors (`control.ts`), synth engine (`engine.ts`), playback scheduler (`scheduler.ts`), multichannel placement (`surround.ts`), note-name/MIDI parsing (`pitch.ts`), named scales (`scale.ts`), noise generation (`noise.ts`), shared types (`types.ts`), public exports (`index.ts`)
